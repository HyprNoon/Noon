...
function navigate(view) {
  const map = { player: '/player', tracks: '/tracks', albums: '/albums', artists: '/artists', library: '/library' }
...
.tab {
  ...
  padding: 10px 12px; text-align: left; border-radius: var(--radius-sm);
...
.tab .icon { font-size: 20px; font-variation-settings: 'FILL' 0, 'wght' 400; flex-shrink: 0; }
.tab.active .icon { font-variation-settings: 'FILL' 1, 'wght' 400; }
.collapsed .tab-label { display: none; }
.collapsed .tab { justify-content: center; padding: 10px 0; }
.collapsed #theme-switcher { display: none; }
...
#theme-cycle:hover { background: var(--surface2); color: var(--text); }
#theme-cycle:active { transform: scale(.95); }
.collapsed #theme-cycle { display: flex; }
